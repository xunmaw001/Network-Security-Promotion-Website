            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">法律法规收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="falvgaguiName" id="falvgaguiName" placeholder="名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../falvgagui/detail.jsp?id='+item.falvgaguiId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.falvgaguiPhoto?item.falvgaguiPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../falvgagui/detail.jsp?id='+item.falvgaguiId)"><span>{{item.falvgaguiName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.newMoney"><span style="color: red;">{{item.newMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteFalvgaguiCollection(item.id)">删除 {{item.falvgaguiName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 falvgaguiCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            falvgaguiCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteFalvgaguiCollection(id){
                var paramArray = [];
                paramArray.push(id);
                layui.http.requestJson(`falvgaguiCollection/delete`, 'post',paramArray, function (res) {
                    if (res.code == 0) {
                        layer.msg('删除成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#falvgaguiName').val() != null && jquery('#falvgaguiName').val() != "" && jquery('#falvgaguiName').val() != "null" ) {
                param['falvgaguiName'] = jquery('#falvgaguiName').val();
            }
                                                                                                
            // 当前表
                                // 获取列表数据
            http.request('falvgaguiCollection/list', 'get', param, function(res) {
